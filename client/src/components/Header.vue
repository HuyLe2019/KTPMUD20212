<template>
  <header class="grid grid-cols-3 justify-between px-6 py-4 bg-white border-b-4 border-indigo-500">
    ...
    <div class="flex items-center justify-self-end">
      <div class="relative">
        ...
        <transition
          ...
          <div v-show="dropdownOpen" class="absolute right-0 z-20 w-48 py-2 mt-2 bg-white rounded-md shadow-2xl">
            <a class="block px-4 py-2 text-sm text-gray-700 hover:bg-indigo-600 hover:text-white">{{ `Hi ${currentUser.username}` }}</a>
            <router-link class="block px-4 py-2 text-sm text-gray-700 hover:bg-indigo-600 hover:text-white" to="/profile"> Profile </router-link>
