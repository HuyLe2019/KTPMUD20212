...
export default defineComponent({
  ...
  async created() {
    ...
    try {
      const { data } = await CustomerService.getCustomer(this.$axios, this.customerId)
      this.customer = data
    } catch (e) {
  ...
  methods: {
    async submit() {
      ...
      this.$store.dispatch('startLoading')
      try {
        const res = await CustomerService.editCustomer(this.$axios, this.customerId, this.customer)
        if (res.success) {
